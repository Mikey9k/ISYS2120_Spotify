...
DROP FUNCTION IF EXISTS mediaserver.addsong(text,text,character varying,integer,text,integer,text);
...
-- to add album_ID & Artist name check/add*******************************************
...
RETURNS int AS
$BODY$
    --insert storage location, get media_id
	WITH ins1 AS (
        INSERT INTO mediaserver.mediaItem(storage_location)
        VALUES (location)
        RETURNING media_id
        )
        --insert decription metadata, get md_id
        , ins2 AS (
        INSERT INTO mediaserver.metadata (md_type_id,md_value)
        ...
        --insert into song table, song_id,title,length
        ,ins4 AS (
        INSERT INTO mediaserver.Song
        SELECT media_id, title, songlength FROM ins1
        RETURNING song_id as song_id_num
        )

        --! artist is selected from dropdown so we don't insert a new genre
        ,ins4a2 AS (
        INSERT INTO mediaserver.Song_Artists
        SELECT song_id_num, artistid FROM ins4
        returning performing_artist_id as pa_id
        )

        --insert artwork metadata
        ,ins6 AS (
        INSERT INTO mediaserver.metadata (md_type_id,md_value)
        ...
        ,ins7 AS (
        INSERT INTO mediaserver.metadata (md_type_id,md_value)
        SELECT md_type_id, artistid
        FROM mediaserver.MetaDataType where md_type_name = 'artist_id'
        RETURNING md_id as artist_md_id
        )
        ...
        ,ins10 AS(
        INSERT INTO mediaserver.MediaItemMetaData
        SELECT media_id, artwork_md_id FROM ins1, ins6
        )
        ,ins11 AS(
        INSERT INTO mediaserver.MediaItemMetaData
        SELECT media_id, pa_id FROM ins1, ins4a2
        )

        select max(song_id) from mediaserver.song
